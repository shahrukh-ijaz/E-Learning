        ...
        <View style={styles.body}>
          <Content>
            <List>
              ...
              {lessons && lessons.map((object, index) => (
                <Fragment key={index}>
                  <ListItem itemHeader style={styles.itemHeader}>
                    <Text style={styles.itemHeaderText}>{object.name}</Text>
                  </ListItem>
                  {object.videos && object.videos.length && object.videos[0].map((videoObject, key) => (
                    <ListItem key={key} onPress={()=> this.props.navigation.navigate("LessonVideo",{link: videoObject.link})}>
                      <Text style={styles.itemText}>{videoObject.title}</Text>
                    </ListItem>
                  ))}
                </Fragment>
              ))}
